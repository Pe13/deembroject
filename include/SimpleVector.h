...
class SimpleVector {
 ...
 public:
  static SimpleVector createEmpty() { return SimpleVector(0, 0, 0); }

  static SimpleVector createCartesian(const T x, const T y, const T z) { return SimpleVector(x, y, z); }
  ...
  auto operator*(const SimpleVector &other) const { return x_ * other.x_ + y_ * other.y_ + z_ * other.z_; }
  ...
  [[nodiscard]] double magnitude() const { return std::sqrt(x_ * x_ + y_ * y_ + z_ * z_); }

  [[nodiscard]] double tMagnitude() const { return std::sqrt(x_ * x_ + y_ * y_); }
